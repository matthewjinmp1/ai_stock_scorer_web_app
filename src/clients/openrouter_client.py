...
class SmartRateLimiter:
    """
    A thread-safe rate limiter that tracks request counts over time
    to stay just below a specified limit.
    """
    ...
    def wait_if_needed(self):
        """
        Blocks the current thread if we are approaching the RPM limit.
        """
        if self.rpm <= 0:
            return
        ...
        with self.lock:
            ...
            if len(self.requests) >= self.rpm:
                # Calculate sleep time to stay just under the limit
                # Find the oldest request in the window and wait until it's > 60s old
                oldest = self.requests[0]
                sleep_time = 60 - (now - oldest) + 0.1  # Add buffer
                if sleep_time > 0:
...
class OpenRouterClient:
    ...
    def __init__(self, api_key: Optional[str] = None, rate_limiter: Optional[SmartRateLimiter] = None):
        ...
        Args:
            ...
            rate_limiter: An optional SmartRateLimiter instance shared across threads.
        """
    ...
    def chat_completion_with_tokens(
        ...
        **kwargs
        ...
        try:
            ...
            # Check for reasoning/thinking tokens specifically
            # Some models provide this in completion_tokens_details.reasoning_tokens
            reasoning_tokens = 0
            if hasattr(usage, 'completion_tokens_details') and usage.completion_tokens_details:
                reasoning_tokens = getattr(usage.completion_tokens_details, 'reasoning_tokens', 0)
            
            # Or directly on usage for some providers
            ...
            # or they might be reported as the difference between total and accounted tokens
            accounted_tokens = prompt_tokens + completion_tokens
            
            # Use the larger of the two (calculated vs reported)
            thinking_tokens = max(reasoning_tokens, (total_tokens - accounted_tokens) if total_tokens > accounted_tokens else 0)
            
            if thinking_tokens > 0:
                # Add thinking tokens to token_usage
                token_usage['thinking_tokens'] = thinking_tokens
                # If thinking_tokens were NOT part of completion_tokens, we need to add them 
                # to completion_tokens for pricing (since output tokens = actual completion + reasoning)
