...
TOP_DB = os.path.join(PROJECT_ROOT, "web_app", "top_scores.db")
...
def analyze_cross_db_correlation():
    ...
    # Get all Mimo data from top_scores.db
    mimo_condition = "model IN ('xiaomi/mimo-v2-flash', 'xiaomimimo/mimo-v2-flash', 'xiaomi/mimo-v2-flash:free')"
    mimo_all_data = get_latest_data(TOP_DB, mimo_condition)
    print(f"Loaded {len(mimo_all_data)} Mimo records from {os.path.basename(TOP_DB)}")
